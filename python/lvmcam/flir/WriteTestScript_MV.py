...
# Max exposure time HCG mode  VALUES for MV 13 Feb 21
eTHmax = [3.5, 2.0, 0.6, 0.2, 0.06, 0.015, 0.01]
# Max exposure time LCG mode     after Black Clamping off
eTLmax = [20.0, 10.0, 3.5, 1.0, 0.35, 0.1, 0.07]
...
for g in range(nGains):  # and gains...
    ...
    for t in range(nTim):  # and expTime
        ...
        Name = nmRt + "_L_" + str(gain[g]) + "_" + str(t)
        # One line of the script
        scrLine = Name + " LCG " + str(gain[g]) + " " + \
            str("{:.3e}".format(expT[t] * 1.0E6) + " " + nFrm)
...
for g in range(nGains):  # and gains...
    ...
    for t in range(nTim):  # and expTime
        ...
        Name = nmRt + "_H_" + str(gain[g]) + "_" + str(t) + \
            ".dat"           # Name of data file to write
        # One line of the script
        scrLine = Name + " HCG " + str(gain[g]) + " " + \
            str("{:.3e}".format(expT[t] * 1.0E6) + " " + nFrm)
