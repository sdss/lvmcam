...
for i in range(len(inList)):     # Step through one line at a time
    ...
    if inList[i][0] != "#":          # Ignore comment lines, if any
        ...
        fName, gainConv, gain, expTime, nFrames = vals[0], vals[1], float(
            vals[2]), float(vals[3]), int(vals[4])

        print("")
        print("Working on file ", i + 1, " with ", nFrames,
              " frames and output ", fName)                 # Feedback
        print("  GainMode: ", gainConv, "  Gain: ", gain,
              "  Exposure Time: ", expTime / 1.0E6, " sec")
        ...
        volts, current, power, temperature = FU.FLIR_Power(
            cam, dev, False)   # Get power temperature info
        ...
        logLine = fName + " " + gainConv + " " + str(gain) + " " + str("{:.3e}".format(expTime / 1.0E6)) + " " + str(
            nFrames) + " " + str("{:.3f}".format(temperature)) + " " + str("{:.3e}".format(mean)) + " " + str("{:.3e}".format(variance))
