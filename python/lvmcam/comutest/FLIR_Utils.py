...
def Standard_Settings(cam, dev, verbose):
    ...
    # Frame rate (Hz). I don't think that this has any effect (for single frames)
    frameRate = 1.0
    ...
    cam.set_exposure_time_auto(Aravis.auto_from_string(
        'Off'))                         # Auto-exposure off
    # Also auto-gain
    cam.set_gain_auto(Aravis.auto_from_string('Off'))
    ...
    # 12-bit ADC - only option for this camera
    cam.set_string('AdcBitDepth', 'Bit12')
    # Which temperature sensor - only option for this camera
    cam.set_string('DeviceTemperatureSelector', 'Sensor')

    # Turn off auto-correction of (known) bad pixels
    cam.set_boolean('DefectCorrectStaticEnable', False)

    # CHECK - seems to correct low signal clamping
